...
class FileUploadFilter(logging.Filter):
    ...
    This filter inspects the log message for a pattern indicating an embedded file. If
    the pattern is found, the base64 data is replaced with a placeholder instead of
    suppressing the entire log record.
    """
    ...
    def __init__(self, pattern_to_find: str = "'type': 'file', 'file':"):
        ...
        Args:
            pattern_to_find (str): Pattern to search for in log messages to
                identify records that need redaction.
        """
        super().__init__()
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if message != original_message:
            record.msg = message
            record.args = ()  # The message is now a string, so clear args.
...
class Logger:
    ...
    def shutdown(self):
        ...
        # Remove the queue handler from the root logger to prevent further
        # logging attempts
        if self.queue_handler in self.root_logger.handlers:
            self.root_logger.removeHandler(self.queue_handler)

    def stop_logging(self) -> None:
        """Stop the logging listener and clean up resources."""
        if self.listener:
            self.listener.stop()
            logging.info("Logging listener stopped.")
        # Remove the queue handler from the root logger to prevent further
